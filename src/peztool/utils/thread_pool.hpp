...
struct ThreadPool final
{
    ...
    template<typename TCallback>
    void dispatch(size_t element_count, TCallback&& callback)
    {
    ...
    template<typename TContainer, typename TCallback>
    void map(TContainer& container, TCallback&& callback)
    {
        dispatch(container.size(),[&callback](uint32_t start, uint32_t end) {
